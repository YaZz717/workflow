...
| **5 — livrée** | Documents : navigateur `/documents` (portée organisation / général / projet, dossiers, recherche, archivés), éditeur `/documents/[id]` en Markdown (GFM) avec aperçu en direct, archivage/restauration, suppression, pièces jointes (upload validé serveur, réutilise `lib/upload.ts` et la route `/api/attachments/[id]` généralisée). Dossiers : création / renommage / suppression (les documents sont détachés, pas supprimés). Recherche globale étendue au **contenu des documents** et aux **commentaires**, avec extrait de texte |
| **6 — livrée** | **Préférences de notification** par utilisateur (`User.notificationPrefs` JSON, 8 types désactivables, `notify()` filtre les destinataires). **Temps réel** : `GET /api/notifications/stream` (SSE, polling serveur 10 s) → la cloche se met à jour sans rechargement. **Export CSV** : `/api/export/{projects,tasks,time}` (borné aux projets visibles). **Déploiement** : `output: standalone`, `Dockerfile` multi-stage (migrate + serveur), `docker-compose.prod.yml`, `docs/DEPLOYMENT.md`. **Tests e2e authentifiés** (Playwright, compte de démo). |
